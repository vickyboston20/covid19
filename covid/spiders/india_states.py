...
class IndiaStatesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        rows = response.xpath("//div[@id= 'cases']"
                              "//table[@class='table table-striped table-dark']/tbody/tr[position()<=27]")
        total_rows = response.xpath("//div[@id= 'cases']"
                                    "//table[@class='table table-striped table-dark']/tbody/tr[position()=28]")
        for row in rows:
        ...
        for total in total_rows:
            total_india = total.xpath(".//td[1]/strong/text()").get()
